...
from modelling.data_processing import process_data
from modelling.evaluation import evaluate_model
from modelling.modelling import train_model
from modelling.raw_data_preparer import prepare_data

def start(data_file_path: str, labels_data_path: str, train_data_ratio: float, threshold: float, seed: int):
    ...
    X_train, y_train, X_test, y_test = process_data(reads_data, labels_data, train_data_ratio, seed)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Execute the full ML training pipeline.')
    parser.add_argument('--data_file_path', type=str, help='Path to the gzipped dataset json file.')
    ...
    start(args.data_file_path, args.labels_data_path, args.train_data_ratio, args.threshold, args.seed)
