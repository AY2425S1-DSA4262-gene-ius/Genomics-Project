...
def prepare_data(data_file_path: str):
    print(f'[raw_data_preparer] - INFO: Initialising the processing of reads: {data_file_path}')

    reads_data = []
    transcript_count = 0
    with gzip.open(data_file_path, 'rt') as transcript_lines:
        print(f'[raw_data_preparer] - INFO: Formatting json file, please be patient...')
        for transcript_line in transcript_lines:
            transcript_count += 1
            if transcript_count % 30000 == 0:
                print(f'[raw_data_preparer] - DEBUG: Currently processing {transcript_count}th transcript')
    ...
    print(f'[raw_data_preparer] - INFO: All transcripts processed, converting to CSV (this may take a while)...')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Process transcript data from the raw compressed dataset json file.')
