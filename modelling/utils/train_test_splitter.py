...
def split_data(merged_data: pd.DataFrame, train_data_ratio: float = 0.8, seed: int = 42):
    ...
    train_genes, test_genes = unique_genes[:int(len(unique_genes) * train_data_ratio)], unique_genes[int(len(unique_genes) * train_data_ratio):]
